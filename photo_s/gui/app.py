...
from .panels import (MaskPanelMixin, ReviewPanelMixin,
                     SettingsPanelMixin, WorkflowDialogsMixin,
                     WatchPanelMixin)
...
class PhotoSApp(ReviewPanelMixin, WorkflowDialogsMixin,
                  SettingsPanelMixin, MaskPanelMixin,
                  WatchPanelMixin):
    ...
    def __init__(self, root):
        ...
        self._gui_state = _gui_state = self._load_gui_state()
    ...
    def _bind_pane_sash_restore(self, pane, value):
        """Restore a sash position once the pane is actually mapped —
        after_idle alone is too early (the module may not be shown at
        startup, and sashpos on an unmapped pane does nothing)."""
        done = [False]

        def _try(_event=None):
            if not done[0]:
                done[0] = self._restore_pane_sash(pane, value)

        pane.bind("<Map>", _try, add=True)
        pane.after_idle(_try)

    def _reapply_pane_sash(self, pane, value, tries=12):
        """Re-apply a sash position after a pane re-add — a later geometry
        pass can re-clamp the sash right after the first set, so keep
        re-applying for the whole ~600ms window instead of stopping at the
        first momentary success."""
        try:
            ...
            pass
        if tries > 0:
            pane.after(50, lambda: self._reapply_pane_sash(pane, value,
                                                           tries - 1))
    ...
    def _save_gui_state(self):
        ...
        save_state({
            ...
            # v2.6 结构项: draggable sash positions (None until mapped)
            "develop_sash": self._pane_sash(getattr(self, "_dev_main", None)),
            "export_sash": self._pane_sash(getattr(self, "_export_main",
                                                   None)),
        })

    def _pane_sash(self, pane):
        """Current sash position of a PanedWindow (None when unmapped)."""
        try:
            return int(pane.sashpos(0))
        except (AttributeError, tk.TclError, ValueError):
            return None

    def _restore_pane_sash(self, pane, value):
        """Apply a persisted sash position. Returns True when applied —
        sashpos before the pane is mapped silently does nothing, so the
        caller keeps retrying on <Map> until it sticks (once)."""
        try:
            if value and int(value) > 0:
                pane.sashpos(0, int(value))
                return int(pane.sashpos(0)) == int(value)
        except (AttributeError, tk.TclError, TypeError, ValueError):
            pass
        return False
    ...
    def _on_main_close(self):
        ...
        try:
            self._tools_close_panel()  # runs the hosted panel's cleanup
        except Exception:
            pass
    ...
    def _build_ui(self):
        ...
        # v2.6 结构项: draggable sash between them (persisted as
        # export_sash), mirroring the Develop main row
        export_row = ttk.PanedWindow(self._module_frames["export"],
                                     orient="horizontal")
        export_row.pack(fill="both", expand=True)
        self._export_main = export_row
        queue_pane = tk.Frame(export_row, bg=COLORS["bg"])
        export_row.add(queue_pane, weight=1)
        self._build_export_queue(queue_pane)
        export_col = tk.Frame(export_row, bg=COLORS["bg"],
                              width=SETTINGS_WIDTH)
        export_col.pack_propagate(False)
        export_row.add(export_col, weight=0)
        self._bind_pane_sash_restore(export_row,
                                      self._gui_state.get("export_sash"))
    ...
    # v2.6: watch is panelized inline (toggle); the rest still launch
    # workflow dialogs — in-place non-modal panels land per-tool batch
    # (plan §4.2).
    TOOLS_CARDS = (
        ...
        ("watch_title", "tools_desc_watch", "_tools_open_watch"),
    ...
    # v2.6 非模态化：这些工具卡内嵌为 Tools 常驻面板（对话框方法接受
    # host= 参数；review 本就内嵌 Library，走直通路由）
    _PANELIZED_TOOLS = frozenset((
        "_show_dedup", "_show_compare", "_show_gallery_export",
        "_show_contact_sheet", "_show_cull", "_show_hash", "_show_hdr",
        "_show_rename", "_show_presets", "_show_analysis",
    ))
    ...
    def _build_tools_panel(self, parent):
        ...
        v2.6 结构项（非模态化首批）: panelized tools live in an inline host
        under the grid — click the card again to close. Watch goes first
        (the autopilot cockpit wants to stay open while you work); the
        remaining cards keep their dialogs until their own batches."""
        ...
        for i, (title_key, desc_key, opener) in enumerate(self.TOOLS_CARDS):
            ...
            if opener in self._PANELIZED_TOOLS:
                launch = (lambda op=opener: self._tools_toggle_panel(op))
            else:
                launch = getattr(self, opener)
            FlatButton(
        ...
        self._tools_panel_host = tk.Frame(holder, bg=COLORS["bg"])
        self._tools_panel_open = None  # hosted opener method name

    # ── v2.6 结构项: inline tool panels (non-modal by construction) ───────

    def _tools_toggle_panel(self, opener):
        """Host a panelized tool inline; clicking its card again closes."""
        host = getattr(self, "_tools_panel_host", None)
        if host is None:
            return
        if getattr(self, "_tools_panel_open", None) == opener:
            self._tools_close_panel()
            return
        self._tools_close_panel()  # drop a different tool's panel first
        host.pack(fill="both", expand=True, pady=(10, 0))
        self._tools_panel_open = opener
        getattr(self, opener)(host=host)

    def _tools_close_panel(self):
        """Unhook the hosted panel: run its cleanup (e.g. stop a watcher),
        destroy the widgets, unpack the host."""
        host = getattr(self, "_tools_panel_host", None)
        if host is None:
            return
        hook = getattr(host, "close_hook", None)
        if hook is not None:
            try:
                hook()
            except Exception:
                pass  # cleanup is best-effort; closing must always succeed
            host.close_hook = None
        for child in host.winfo_children():
            try:
                ...
                pass
        host.pack_forget()
        self._tools_panel_open = None

    def _tools_open_watch(self):
        """Watch card: panelized (v2.6) — toggle the inline autopilot
        cockpit instead of a popup dialog."""
        self._tools_toggle_panel("_show_watch")
    ...
    def _build_develop_panel(self, parent):
        ...
        # Main row: viewer card + analysis sidebar. v2.6 结构项: the
        # fixed-width sidebar became a draggable ttk.PanedWindow sash
        # (position persisted as gui_state develop_sash; the mask editor
        # hides the sidebar pane instead of repacking it)
        ...
        viewer_card = tk.Frame(left, bg=COLORS["card"], bd=0,
                               highlightthickness=0)
        viewer_card.pack(side="left", fill="both", expand=True)
        self._dev_viewer_card = viewer_card
        # v2.5: 蒙版编辑器宿主（左窗格内 pack 切换见 _dev_mask_apply_layout）
        self._dev_mask_host = tk.Frame(left, bg=COLORS["bg"])

        self._bind_pane_sash_restore(main, self._gui_state.get("develop_sash"))
    ...
    def _show_hdr(self, host=None):
        ...
        if host is not None:
            win = self._hosted_frame(host, self._t("hdr_title"))
        else:
            win = tk.Toplevel(self.root)
