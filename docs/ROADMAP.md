...
**P1 — Library 语义搜索 + 过滤增强 + Develop 智能建议按钮**：✅ 已落地
（2026-09-18，随本批发版）：
- [x] Library 过滤框升级：文件名旁加语义搜索（有索引用之，无索引引导
      构建；无插件时 hist84 不支持文本查询需明确提示）；按评级/已调徽标/
      格式过滤（另加颜色标签筛选与 P2 联动；组合选择按下标存储，语言
      切换安全；语义查询 worker 线程 + UiBus 回投，不卡 UI）
- [x] Develop「智能建议」按钮：suggest 参数一键应用（零依赖，可撤销），
      兼作 AI 调色缺插件时的兜底（中性图明确提示「指标均衡」）

**P2 — 反馈补全**：✅ 已落地（2026-09-18）：
- [x] 导出完成显示 audit 通过率与失败原因下钻（状态栏通过率 + 摘要窗
      逐文件原因 + 「打开输出目录」入口；Export 选项可关，默认开）
- [x] 颜色标签（XMP label 写出侧已支持，GUI 无入口；评级筛选联动）——
      EXIF UserComment 协议扩 `label` 字段（写出/撤销/读取与 rating 同款
