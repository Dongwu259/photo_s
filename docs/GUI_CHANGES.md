...
## 16. v2.4.0 — 所见即所得 + 大库性能 + AI 调色 GUI

### 16.1 三处所见统一（WYSIWYG）

同一张照片在 Develop、⌘P 预览、审查灯箱、蒙版画布长得一样：

- **⌘P 预览**：渲染经 `_per_file_overlay`（逐照片覆盖层 + 蒙版，与批量导出同一路径）；
  staleness 键改为有效 options——覆盖层变化（粘贴/AI 调色落地）会触发重渲；
  「已调」徽标替代旧的「预览不含逐照片蒙版」提示
- **审查灯箱**：有覆盖层/蒙版的照片异步走管线渲染（`_render_adjusted_async`
  共享助手，dev bus 回投，令牌防串台）；info 行追加「已调」徽标
- **蒙版画布**：底图走「仅调色」注入（`include_masks=False`，避免与画布实时
  蒙版叠加双重应用；按 path 缓存），蒙版叠加仍由画布实时绘制

### 16.2 AI 自动调色（官方插件）进 Develop

- 底栏「AI 调色」按钮 + 强度下拉（1.0/0.8/0.6/0.4/0.2，默认 0.8）
- worker 线程调 `photo_s_plugin_auto-tone.auto_tone(path, strength, render=False)`；
  预测的 9 参数写入逐照片覆盖层（exposure→ev 映射，其余同名），并入撤销历史，
  滑杆即时反映——可微调、可 ⌘Z、导出自动生效
- 缺插件 → 安装指引（含规则版 `photo-s suggest` 替代）；推理失败 → 透出插件
  自带的清晰错误；busy 标志防重入

### 16.3 Library VirtualGrid（大库性能）

- 文件列表从「每行 7 widget 全量构建」改为**固定行高 canvas 虚拟列表**：
  数据模型 + 仅可视窗口物化；勾选/评级/选中状态存 `_checked`/`_selected_rows`，
  绘制时读取（勾选=重绘一遍，非重建）
- 5k 张基准：首绘 20.9ms / 40 个 canvas 项（旧方案 ≈3.5 万 widget）、
  翻页 0.3ms、全量刷新（模型重建）669ms
- 关键坑：yscrollcommand 在每次 scrollregion 更新后重发 → 无条件重绘构成
  无限乒乓；行画在绝对 y 上画布原生滚动，**仅可视行窗口变化才重绘**
- 行内新增：评级星标（EXIF 懒读缓存）、「已调」徽标点、占位缩略图 +
  可视行批量解码（RAW 仍走 worker）

### 16.4 键盘（对齐灯箱）

- 图库：1-5 评级 / P 清除（拒绝）/ Enter 在修图中打开 / ⌫·Delete 移除所选
  ——根级绑定 + 双守卫（仅 Library 模块生效；焦点在 Entry/Text 等输入件时失效）
- `?`（或 ⌘?）任意位置打开快捷键表

### 16.5 Develop 前后对比

- 「对比」按钮循环 关→分割→并排→关；overlay canvas 覆盖原 Label（旧缝不动）
- 分割模式：PIL 按分割比例合成 + 分割线/把手，点击/拖动移动（节流重绘）
- 并排模式：等高缩放并排 + 分隔线

### 16.6 易用性

- 设置搜索：设置面板顶部搜索框跨 4 个 Tab + Develop 调整面板按标签查找，
  命中高亮 + 自动切 Tab/模块 + 滚动入视野；清空恢复
- 预设浏览器（Develop 侧栏）：悬停实时预览——预设 options 直接替换预览渲染
  的 sig（不碰滑杆/覆盖层/撤销史），移开即恢复；点击经 `_apply_options_to_ui`
  正式套用
- 首跑引导卡：gui_state `first_run_done` 标记，三步上手 + 快捷键指路

### 16.7 测试

- `tests/test_gui_v24.py` 32 项：AI 调色（覆盖层/撤销/强度/合并/缺插件/失败/
  重入）· WYSIWYG 渲染（覆盖层生效/蒙版开关/灯箱接线）· VirtualGrid（模型/
  裁剪/勾选/命中/5k 项数）· 键盘守卫（模块/输入焦点）· 对比（循环/拖动/回落）·
  设置搜索 · 预设悬停与应用 · 首跑卡持久化
- 全量 1308 通过；`_row_vars`/`_row_widgets` 旧缝迁移至 `_lib_model`/`_checked`

## 17. v2.5 结构归位：蒙版画布进 Develop、审查灯箱进 Library

> v2.4 立项时明确推迟的两项布局归位（ROADMAP 在案）。全部走「共享构建器
> + 模块内宿主」模式：`_build_mask_workflow` / `_build_review_ui` 接受
> 任意宿主（Toplevel 诊断路径保留），正式入口内嵌进工作区模块——纯
> pack 切换，模块状态零销毁。

### 17.1 蒙版画布升格进 Develop（原 `_open_mask_workflow` 弹窗）

- Develop 工具栏新增「蒙版」按钮（AI 调色旁）：进入蒙版模式 = 主行
  pack 切换（预览+侧栏 ↔ 全宽蒙版编辑器），设置面板「编辑蒙版」按钮
  同入口
- 作用域 = 勾选照片，起始页 = 当前 Develop 选择；每次进入重建（事实源
  是 `_photo_masks` + 全局 masks 变量）
- **编辑即所见**：退出（完成按钮/Esc）时序列化写入 `_photo_masks`，
  预览与导出队列立即重渲——与调色覆盖层同一通道，无「弹窗里改完主
  界面不知道」的窗口
- 键路由：蒙版模式下 ←/→ 翻页、⌘Z 编辑器撤销（优先于逐照片调色
  历史）、Esc 退出；输入框聚焦时不拦截
- 弹窗 OK 路径顺带修复 v1.8 潜伏 bug：字符串型局部调整
  （`curves={...}`/`hsl={...}`）序列化时被 `float()` 转换必崩——旧
  测试只开窗不点 OK 从未触发

### 17.2 审查灯箱并入 Library（原 `_show_review` Toplevel）

- 「审查」按钮 / ⌘E：切到 Library 并把文件网格区换成内嵌灯箱
  （工具栏不动）；关闭按钮 / Esc 退出回网格并刷新行内星标
- 全部能力保留：0-5 星（EXIF 即写）、关键词/标题、拍摄信息编辑、
  评分/关键词筛选、精选淘汰双阈值分拣、「已调」徽标 + 调整后渲染
- 键路由：灯箱激活时 0-5 给灯箱当前照片（不再是网格行）、←/→ 翻页、
  ⌘Z 灯箱内撤销、Esc 退出；起始页 = Library 当前选择

### 17.3 契约与测试

- `_focus_in_text_widget()` 从 `_lib_keys_active` 提取共享（蒙版/
  灯箱/网格三处键守卫复用）
