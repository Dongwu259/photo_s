...
| `xmp-export` | **XMP 写出**（v2.5，LR 双向互通收口）：PhotoS 调整 → LR 可读 `.xmp` sidecar（crs 字段逐项逆映射 + radial/linear 蒙版 → MaskGroupBasedCorrections + xmp:Rating/dc:subject/dc:title）；`--preset/--options JSON/--auto-tone` 取参数（auto-tone 导出真实预测值）；`--embed` 直接内嵌 JPEG APP1 段（LR 对 JPEG 只读内嵌 XMP，sidecar 仅 RAW 生效）；`batch --write-xmp` 批量版 |
