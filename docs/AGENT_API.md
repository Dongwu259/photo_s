...
| `photo-s mcp` | Claude Desktop / MCP 客户端 | stdio MCP server，31 核心工具 + 插件自动注册（需 py3.10+ 与 `photo-s-tools[mcp]`） |
...
`photo-s mcp --list-tools` 返回全部工具及 inputSchema（JSON，不启动服务器；31 核心 + 已装插件的工具）。
